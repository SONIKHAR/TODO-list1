...
class TodoList {
    ...
    <div class="card mb-3 ${isTaskActive ? "" : "completed-task"}" >
      <div class="card-body">
        <div class="form-check mb-2">
          <input class="form-check-input" type="checkbox" onclick="todoList.selectTask('${
            task.id
          }')"/>
        </div>
       <div class="title-container"> <h5 class="card-title task-title">${
         task.title
       }</h5></div>
        <p class="card-text">Status: ${task.status}</p>
        <p class="card-text">Created At: ${task.createdAt}</p>
        <p class="card-text ${isTaskActive ? "d-none" : ""}">Completed At: ${
      task.completedAt
    }</p>
        <div class="justify-content-end d-flex">
          <button class="btn btn-success ${
            isTaskActive ? "" : "d-none"
          }" onclick="todoList.changeTaskStatus('${
      task.id
    }', 'done')">Done</button>
      ...
      task.id
          ...
          <button class="btn btn-warning ms-1" onclick="todoList.editTask('${
            task.id
          }')">Edit</button>
          <button class="btn btn-danger ms-1" onclick="todoList.deleteTask('${
            task.id
          }')">Delete</button>
        </div>
      </div>
  ...
  changeTaskStatus = (taskId, taskStatus) => {
    ...
    const newTaskNode = parser
      .parseFromString(taskTemplate, "text/html")
      .querySelector("div");
  ...
  editTask = (taskId) => {
    ...
    const titleContainer = document.querySelector(
      `[data-id="${taskId}"] .title-container`
    );
...
`;
  ...
  cancelTaskEditing = () => {
    const titleContainer = document.querySelector(
      `[data-id="${this.editingTask}"] .title-container`
    );
  ...
  saveTaskEditing = () => {
    ...
    const parser = new DOMParser();
    const editedTaskTitle = parser
      .parseFromString(this.editingTaskOriginalTitle, "text/html")
      .querySelector("h5");
